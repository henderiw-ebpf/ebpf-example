                                       ...
                                       struct iphdr **iphd)
{
  ...
  // verify that the iphdr struct itself fits in the packet payload
	if ( nh->pos + sizeof(struct iphdr) > data_end)
		return -1;

	hdrsize = iph->ihl * 4;
	// verify that this full size fits in the packet 
  // (and adjust the nexthdr pointer accordingly)
